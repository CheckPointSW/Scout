...
def generateGOT(symbol_memcpy, symbol_memset, symbol_malloc, symbol_free, symbol_socket, symbol_bind,
                symbol_listen, symbol_accept, symbol_connect, symbol_recv, symbol_send, symbol_close, project_got = [], is_thumb = False) :
    """Generates the Global Offset Table (GOT) content using the supplied addresses
    
    Args:
        symbol_memcpy (numeric): (virtual) address of the memcpy() function in the hosting address space
        symbol_memset (numeric): (virtual) address of the memset() function in the hosting address space
        symbol_malloc (numeric): (virtual) address of the malloc() function in the hosting address space
        symbol_free (numeric): (virtual) address of the free() function in the hosting address space
        symbol_socket (numeric): (virtual) address of the socket() function in the hosting address space
        symbol_bind (numeric): (virtual) address of the bind() function in the hosting address space
        symbol_listen (numeric): (virtual) address of the listen() function in the hosting address space
        symbol_accept (numeric): (virtual) address of the accept() function in the hosting address space
        symbol_connect (numeric): (virtual) address of the connect() function in the hosting address space
        symbol_recv (numeric): (virtual) address of the send() function in the hosting address space
        symbol_close (numeric): (virtual) address of the close() function in the hosting address space
...
def generateGlobals(scout_vars_size = scout_globals_32_size, project_vars_size = 0) :
    """Configures the globals blob using the supplied sizes
    
    Args:
        scout_vars_size (numeric): size in bytes of the base scout's globals (scout_globals_32_size by default)
